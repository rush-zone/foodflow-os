import { FlowEvent, FlowStatus } from "@/store/useFlowStore";

const steps: { status: FlowStatus; label: string; icon: string }[] = [
  { status: "pending",    label: "Pedido Confirmado",    icon: "📋" },
          ...
          <div key={step.status} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0 transition-all ${
                done    ? "bg-green-500/20 border-2 border-green-500" :
                current ? "bg-brand-primary/20 border-2 border-brand-primary" :
